...
const checkoutCtrl = async (req, res) => {
  try {
    ...
    if (!paymentMethode) {
      return res.status(400).json({ message: "please select payment methode" });
    }
    if (paymentMethode != "destination" && paymentMethode != "card") {
      return res.status(400).json({ message: "wrong spelled" });
    }
  ...
  try {
    ...
    return res.json({ message: "Checkout session created", sessionUrl: session.url });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error 334" });
  }
}

  } catch (error) {
    console.log(console.error);
    res.status(500).json({ message: "internal server error 340" });
  }
};
const stripeCtrl = async (req, res) => {
  const sig = req.headers["stripe-signature"];
  let event;
  try {
    event = stripe.webhooks.constuctEvent(
      req.body,
      sig,
      "whsec_04a8cfa28f692e67745d72973e0a4ce92bb04db24a6bea6b5df0e4eee93f969d"
    );
  } catch (error) {
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed":
        ...
        await conn.query(orderQuery.updateSession,[session.id])
        const [orders] = await conn.query(orderQuery.findIdBysession,[session.id])
